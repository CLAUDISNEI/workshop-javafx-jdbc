...
public class ListaDepartamentoControler implements Initializable, AlterarDadosListeners {
	...
	/*
	 * Criando um dependência para a classe DepartamentoServicos e injetando na
	 * classe
	 */
	...
	public void onBtNovoAction(ActionEvent evento) {
		...
		/*
		 * criado instancia de um Departamento que será utilizada para cadastrar o novo
		 * departamento
		 */
	...
	public void atualizarTableView() {
		 ...
		 * pois assim não teremos nenhum departamento para mostrar
		 */
		if (servicos == null) {
	...
	private void criarJanelaDialogoForm(Departamento departamento, String nomeTela, Stage parentStage) {
		try {
			...
			/*
			 * Criando uma instancia do Departamentocontroler para pegar o controlador da
	...
	private void habilitarBotaoEditar() {
		...
		colunaEditar.setCellFactory(param -> new TableCell<Departamento, Departamento>() {
			...
			protected void updateItem(Departamento dep, boolean empty) {
				...
				setGraphic(botao);
				botao.setOnAction(evento -> criarJanelaDialogoForm(dep, "/gui/DepartamentoForm.fxml",
	...
	protected void removerEntidade(Departamento dep) {
